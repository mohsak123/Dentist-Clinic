  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div className='flex items-start justify-start gap-2 flex-wrap'>
        {isEdit ? (
          <input className='bg-gray-50 text-3xl font-medium mt-4' type="text" value={userData?.firstName} onChange={e => setUserData((prev: any) => ({ ...prev, firstName: e.target.value }))} />
        ) : (
          <p className='font-medium text-3xl text-neutral-800 mt-4'>{userData?.firstName}</p>
        )}

        {isEdit ? (
          <input className='bg-gray-50 text-3xl font-medium mt-4' type="text" value={userData?.lastName} onChange={e => setUserData((prev: any) => ({ ...prev, lastName: e.target.value }))} />
        ) : (
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          <p>Birthday:</p>
          {isEdit ? (
            <input className='max-w-20 bg-gray-100' type="date" onChange={(e) => setUserData((prev: any) => ({ ...prev, dob: e.target.value }))} value={userData?.dob} />
          ) : (
